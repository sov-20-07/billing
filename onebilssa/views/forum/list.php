<div class="standart_menu_slim">
	<div class="gray_line_top">
		<a style="float: left;margin-right: 10px;" href="/onefotossa/forum/<?if($top):?><?=$top?>/<?endif?>">
			<img src="/onefotossa/i/dirup.gif" />
		</a>
		<a href="/<?=Funcs::$cdir?>/<?=Funcs::$uri[1]?>/showadd/?parent=<?=Funcs::$uri[2]?>"><div class="add_section">Добавить</div></a>
	</div>
		...
		<table cellspacing="0" cellpadding="0" width="100%" tab="<?=Funcs::$uri[1]?>" class="spisok">
			<thead>
				<tr>
					...
					<th width="120" class="header" colspan="5">Действия</th>
				</tr>
			</thead>
			<tbody>
			<?foreach($list as $item):?>
				<tr id="row<?=$item['id']?>">
	        		...
	        		<td class="padd_5"><a href="/<?=Funcs::$cdir?>/<?=Funcs::$uri[1]?>/del/?parent=<?=Funcs::$uri[2]?>&id=<?=$item['id']?>" onclick="return confirm('Удалить объект?')" title="удалить"><img border="0" src="/<?=Funcs::$cdir?>/i/ic_trash.gif" alt="удалить"></a></td>
	       			<td class="padd_5"><div style="cursor: move;" title="Переместить" alt="Переместить" src="/<?=Funcs::$cdir?>/i/ic_move.gif" class="mover"></div></td>
				</tr>
			<?endforeach?>
			</tbody>
		</table>
