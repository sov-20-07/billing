		...
		<div class="ltContainer ltFullWidth sizeChild">
			<div class="ltRow jsScrollContainer">
				<div class="link_step_back_holder">
					<a class="link_step_back" href="/<?=Funcs::$cdir?>/user/group/"></a>
				</div>
						...
						<ul class="edit_form_layout jsMoveContainerLI">
							<li class="edit_form_section jsMoveFixed">
								<table class="edit_form_table">
									<tr>
										<td>Название</td>
										<td><div class="input_text_holder"><input type="text" class="input_text" name="name" value="<?=$group['name']?>"></div></td>
									</tr>
									<tr>
												...
												<?if($item['name']):?>
													<li>
														<label class="form_label jsPreviewToggle">
															<input type="checkbox" name="modules[]" value="<?=$path?>" <?if(in_array($path,$group['access']['modules'])):?>checked<?endif?>><span class="form_label_text"><?=$item['name']?></span>
														</label>
													</li>
												<?endif?>
											<?endforeach?>
										...
										<td>
											...
											<?foreach($tree as $item):?>
												<li>
													...
													<?if(count($item['sub'])>0):?>
														<?=View::getRenderEmpty('user/labeltree',array('tree'=>$item['sub'],'left'=>1,'group'=>$group))?>
													<?endif?>
												</li>
											<?endforeach?>
											</ul>
