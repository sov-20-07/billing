<div class="ltCell cell_page-content sizeParent">
	<div class="ltContainer ltFullWidth sizeChild">		
		...
		<?=View::getPluginEmpty('orders/menu',array(
			'statuses'=>$statuses,
			'dealer' => $dealer,
					...
					<table class="sections_table sections_table_zebra">
						...
						<?foreach($list as $item): ?>
							<tr class="sections_table_row">
								...
								<td class="sections_table_cell">
									<?=number_format($item['price']+$item['sale'],0,'',' ')?> $
								</td>
								<td class="sections_table_cell">
									<?=number_format($item['price'],0,'',' ')?> $
								</td>								
								<td class="sections_table_cell">
									<a href="/<?=Funcs::$cdir?>/dealers/editcons/?id=<?=$item['cons']?>"><?=$item['cons']?></a><br>
								</td>
								<td class="sections_table_cell">
									<?=$item['tracking']?>
								</td>
								...
								<td class="sections_table_cell">
									<div class="sections_table_control-panel">
										...
										<a href="/<?=Funcs::$cdir?>/dealers/actdelorder/?id=<?=$item['id']?>&dealer=<?=$dealer['id']?>" onclick="return confirm('Удалить заказ? (это действие нельзя будет отменить)');" title="удалить" class="icon icon_remove"></a>
									</div>
								</td>
							</tr>
						<?endforeach?>
