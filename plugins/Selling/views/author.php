			...
			<div class="cell_page-content_control-panel">		
				<div class="jsTabs">
					<form>
						...
						<div class="input_text_label"><input type="text" name="q" placeholder="Название или ID товара" class="input_text" value="<?=$_GET['q']?>" /></div>
						<div class="input_text_label"><input type="text" name="q" placeholder="Пользователь" class="input_text" value="<?=$_GET['q']?>" /></div>
					...
					<table class="sections_table jsMoveContainerTR" tab="<?=Funcs::$uri[1]?>" page="<?=$_GET['p']?>">
						...
						<?foreach($list as $item):?>
							<tr class="sections_table_row jsMoveElement" id="row<?=$item['id']?>">
								...
								<td class="sections_table_cell"><?=$item['authorprice']?></td>
