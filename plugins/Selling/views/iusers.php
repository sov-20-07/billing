			...
			<div class="cell_page-content_control-panel">		
				<div class="jsTabs">
					<form>
						...
						<a class="button-white" href="/<?=Funcs::$cdir?>/selling/users/">Сбросить</a>
					</form>
					<a class="button" href="/<?=Funcs::$cdir?>/selling/export/<?=Funcs::getFG(NULL,'?');?>">Экспорт</a>
				</div>
			</div>
		</div>
					...
					<table class="sections_table jsMoveContainerTR" tab="<?=Funcs::$uri[1]?>" page="<?=$_GET['p']?>">
						...
						<?foreach($list as $item):?>
							<tr class="sections_table_row jsMoveElement" id="row<?=$item['id']?>">
								...
								<td class="sections_table_cell"><a href="/<?=Funcs::$cdir?>/selling/users/<?=$item['id']?>/"><?=$item['name']?></a></td>
